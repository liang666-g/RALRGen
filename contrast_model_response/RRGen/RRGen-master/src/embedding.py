...
args = Namespace(
    ...
    valid_filename='./data/valid.txt',
    cate_filename='./data/app_category.txt',
    keyword_filename='./data/keyword_dict.json',
    feature_vec_size=20
)
...
class Dataset():
    ...
    def load_cates(self, catepath):
        cate_fr = open(catepath, "r", encoding="utf-8")
    ...
    def load_word_vectors(self):
        ...
        with open(self.filename, "r", encoding="utf-8", errors="ignore") as fp:
            for line in tqdm(fp, leave=False):
                ...
                if len(line) < self.embedding_size + 1:
                    continue

                word = line[0]
                word_to_index[word] = len(word_to_index)

                vec = np.array([float(x) for x in line[1:self.embedding_size + 1]])
                word_vectors.append(vec)

        return word_to_index, word_vectors

    def load_keyword_embedding(self):
        keyword_fr = open(args.keyword_filename, "r", encoding="utf-8")
