...
# oid = 86
# plot_heat_maps(np.delete(lines[oid], np.s_[-1:], axis=1).transpose(), (texts[oid].split('-[split]-')[0]).split(), (texts[oid].split('-[split]-')[1]).split(), oid)
xid = 87
mat = np.delete(np.delete(lines[xid], np.s_[-1:], axis=1).transpose(), np.s_[-12:], axis=1)
mat = np.delete(mat, np.s_[:7], axis=1)
plot_heat_maps(mat, (texts[xid].split('-[split]-')[0]).split(), (texts[xid].split('-[split]-')[1]).split()[7:-12], xid)
# yid = 88
# plot_heat_maps(np.delete(lines[yid], np.s_[-1:], axis=1).transpose(), (texts[yid].split('-[split]-')[0]).split(), (texts[yid].split('-[split]-')[1]).split(), yid)