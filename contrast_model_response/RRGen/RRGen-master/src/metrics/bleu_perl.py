...
def get_ref_cand(opt_fn, pred_fn):
    result_fr = open(os.path.join('/data', 'pred', opt_fn, pred_fn))
    ...
    for result in results:
        terms = result.split("-[split]-")
        references.append(terms[1]+'\n')
        candidates.append(terms[2])

    ref_fn = os.path.join('/data', 'pred', opt_fn, 'ref_'+pred_fn)
    ref_fw = open(ref_fn, 'w')
    ref_fw.writelines(references)
    ref_fw.close()

    cad_fn = os.path.join('/data', 'pred', opt_fn, 'cad_'+pred_fn)
    cad_fw = open(cad_fn, 'w')
    cad_fw.writelines(candidates)
    cad_fw.close()

    # run command
    with open(cad_fn, "rb") as fin:
        output = subprocess.check_output(
            ["perl", "./metrics/multi-bleu.perl", ref_fn],
            stdin=fin
        )
    print(output)
