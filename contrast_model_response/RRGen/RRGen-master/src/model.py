...
if not pred_test:
    ...
    print("attention path =", opts.attention_res_fp)
    print("outtext path =", opts.outtext_fp)
    print("checkpoint path =", getattr(opts, "checkpoint_dir", None))


    model_name = 'seq2seq'
    time_str = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    experiment_name = '{}_{}'.format(model_name, time_str)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_id, batch_data in train_bar:
            train_bar.set_postfix({
                "step": global_step,
                "loss": "{:.4f}".format(float(total_loss)) if total_words > 0 else "0.0000"
            })
            ...
            if global_step % print_every_step == 0:
                ...
                if bleu_score > max_bleu:
                    ...
                    os.makedirs(opts.attention_res_fp, exist_ok=True)
                    os.makedirs(opts.outtext_fp, exist_ok=True)
                    ...
                    print('=' * 100)
                    print('Save checkpoint to "{}".'.format(checkpoint_path))
                    print('Best BLEU: {:.6f}'.format(max_bleu))
                    print('=' * 100 + '\n')

                    att_path = os.path.join(
                        opts.attention_res_fp,
                        "bleu_{:.6f}_{}".format(max_bleu, save_count)
                    )

                    with open(att_path, "wb") as att_fw:
                    ...
                    with open(text_path, "w", encoding="utf-8") as text_fw:
                        for id_text, text in enumerate(out_texts):
                            text_fw.write(test_src_texts[id_text] + "-[split]-" + text + "\n")

                    print("Save attention to:", att_path)
                    print("Save generated texts to:", text_path)
    ...
    ### Start translation
    # --------------------------
    test_fr = open(args.test_filename, "r", encoding="utf-8-sig", errors="replace")
    test_src_texts = []
    test_tgt_texts = []
    # external feature
    test_rates = []
    ...
    dir_name = os.path.join('./data', 'pred', fw_name[-2])
